...
class ContentBlockManager {
  ...
  textStarted = false;
  toolStates = new Map<number, ToolCallState>();
...
export class SSEBuilder {
  ...
  private accumulatedTextParts: string[] = [];
  private accumulatedReasoningParts: string[] = [];
  ...
  emitThinkingDelta(content: string): string {
    ...
    return this.contentBlockDelta(
      this.blocks.thinkingIndex,
      "thinking_delta",
      content
    );
  }
  ...
  *closeAllBlocks(): Generator<string> {
    ...
    for (const [toolIndex, state] of this.blocks.toolStates.entries()) {
      if (state.started) yield this.stopToolBlock(toolIndex);
    }
  }
  ...
  *emitError(errorMessage: string): Generator<string> {
  ...
  estimateOutputTokens(): number {
    ...
    for (const state of this.blocks.toolStates.values()) {
      if (state.started) toolTokens += 50;
    }
