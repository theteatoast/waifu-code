...
export function isQuotaCheckRequest(requestData: MessagesRequest): boolean {
  if (
...
export function isTitleGenerationRequest(requestData: MessagesRequest): boolean {
...
export function isPrefixDetectionRequest(
...
export function isSuggestionModeRequest(requestData: MessagesRequest): boolean {
  for (const msg of requestData.messages) {
    if (msg.role === "user") {
      const text = extractTextFromContent(msg.content);
      if (text.includes("[SUGGESTION MODE:")) return true;
    }
  }
  return false;
}

export function isFilepathExtractionRequest(
  requestData: MessagesRequest
  ...
  const systemText = requestData.system
    ...
    : "";
  const systemHasExtract =
    systemText.includes("extract any file paths") ||
